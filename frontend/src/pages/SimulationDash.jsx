...
export default function SimulationDash() {
  ...
  const [placeMode, setPlaceMode] = useState(null)   // 'patient'
  const [pendingPat, setPendingPat] = useState(null)  // { lat, lon }
  ...
  const doRegister = async (scenario=null) => {
    ...
    try {
      ...
      if(res.dispatch?.error) {
        show(`Patient registered — ${res.dispatch.error}. Add resources in Control Centre.`,'warning')
      } else {
        show(`✓ ${name} registered → ${res.dispatch?.assigned_hospital?.name} assigned, ambulance dispatched!`)
      }
  ...
  const handleMapClick = (lat, lon) => {
    if (placeMode === 'patient') setPendingPat({ lat, lon, type:'patient' })
  }

  const doApply = async () => {
    try { await updateSimulation(speed,sens); show('Settings applied to running simulation') }
    catch(e) { show(e.message,'error') }
  }
  const doStop = async () => {
    try { await stopSimulation(); show('Simulation stopped','warning') }
    catch(e) { show(e.message,'error') }
  }
  ...
  return (
    <Layout title="Simulation" subtitle="Demo & testing — not part of the operational system">
      <div style={{ display:'flex', height:'100%', overflow:'hidden' }}>
        ...
        <div style={{ width:310, background:'white', borderRight:'1px solid var(--border)', display:'flex', flexDirection:'column', overflow:'auto', flexShrink:0 }}>
          ...
          <div style={{ margin:12, padding:'10px 12px', background:'var(--yellow-lt)', border:'1px solid #fde68a', borderRadius:9 }}>
            <div style={{ fontWeight:700, fontSize:13, color:'#92400e', marginBottom:3, fontFamily:"'Rajdhani',sans-serif", letterSpacing:'0.03em' }}>ℹ️ Demo Tool</div>
            <div style={{ fontSize:12, color:'#78350f', lineHeight:1.6 }}>
              This screen is for demonstration only. Register a patient to trigger auto-dispatch. Watch it live in Control, Ambulance & Hospital dashboards.
            </div>
          </div>
          ...
          <div style={{ padding:'0 12px 14px' }}>
            <div style={{ border:`1.5px solid ${isRun?'#fed7aa':'var(--border)'}`, borderRadius:10, padding:12, background:isRun?'var(--orange-lt)':'var(--surface2)' }}>
              <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom: isRun?10:0 }}>
                ...
                <span style={{ fontWeight:600, fontSize:13, color:isRun?'var(--orange)':'var(--text2)' }}>
                  {isRun?'Running':'Idle'}
                </span>
              </div>
              {isRun && simSt && (
          ...
          <div style={{ padding:'0 12px 14px', display:'flex', flexDirection:'column', gap:8 }}>
            ...
            <div style={{ fontSize:11, color:'var(--text3)', lineHeight:1.6 }}>
              📍 Location auto-assigned · System finds nearest ambulance & hospital automatically
            </div>
            {/* Location picker */}
            <div style={{ border:`1px solid ${pendingPat?'var(--green2)':'var(--border)'}`, borderRadius:8, padding:10, background:pendingPat?'var(--green-lt)':'var(--surface2)' }}>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:pendingPat?6:0 }}>
                <div>
                  <div style={{ fontSize:12, fontWeight:600, color:pendingPat?'var(--green)':'var(--text2)' }}>
                    {pendingPat ? '✓ Accident location set' : '📍 Set accident location'}
                  </div>
                  {pendingPat && (
                    <div style={{ fontSize:11, color:'var(--text3)', fontFamily:"'JetBrains Mono',monospace" }}>
                      {pendingPat.lat.toFixed(4)}, {pendingPat.lon.toFixed(4)}
                    </div>
                  )}
                </div>
                <button
                  className={`btn btn-sm ${placeMode==='patient'?'btn-danger':'btn-ghost'}`}
                  onClick={()=>{ setPlaceMode(m=>m==='patient'?null:'patient'); if(placeMode==='patient') setPendingPat(null) }}>
                  {placeMode==='patient' ? '✕ Cancel' : pendingPat ? '📍 Change' : '📍 Click Map'}
                </button>
              </div>
              {placeMode==='patient' && (
                <div style={{ fontSize:11, color:'var(--blue2)', fontWeight:500, marginTop:6 }}>
                  👆 Click anywhere on the map to drop the accident pin
                </div>
              )}
              {!pendingPat && placeMode!=='patient' && (
                <div style={{ fontSize:11, color:'var(--text3)', marginTop:pendingPat?0:6 }}>
                  Click "Click Map" then tap the accident location on the map
                </div>
              )}
            </div>
            <button className="btn btn-danger" onClick={()=>doRegister()} disabled={loading||!patName||!patAge} style={{ justifyContent:'center' }}>
              {loading?'Registering…':'🚨 Register & Auto-Dispatch'}
            </button>
          </div>
          ...
          <div style={{ padding:'0 12px 14px', display:'flex', flexDirection:'column', gap:5 }}>
            {SCENARIOS.map(s=>(
              <button key={s.label} className="btn btn-ghost" disabled={loading} onClick={()=>doRegister(s)}
          ...
          <div style={{ padding:'0 12px 14px' }}>
            ...
            <input type="range" min={0.5} max={5} step={0.5} value={speed} onChange={e=>setSpeed(parseFloat(e.target.value))}
              style={{ width:'100%', accentColor:'var(--blue2)', marginBottom:8 }}/>
            <div style={{ display:'flex', gap:4 }}>
              {[0.5,1,2,3,5].map(v=>(
                <button key={v} onClick={()=>setSpeed(v)} style={{
                  flex:1, padding:'5px 0', border:`1px solid ${speed===v?'var(--blue2)':'var(--border)'}`,
                  borderRadius:6, fontSize:11, fontWeight:600, cursor:'pointer', fontFamily:'inherit',
                  background:speed===v?'var(--blue-lt)':'white', color:speed===v?'var(--blue2)':'var(--text2)',
                }}>{v}×</button>
              ))}
            </div>
          </div>
          ...
          <div style={{ padding:'0 12px 14px' }}>
            ...
            <input type="range" min={0.01} max={0.2} step={0.01} value={sens} onChange={e=>setSens(parseFloat(e.target.value))}
              style={{ width:'100%', accentColor:'var(--green2)', marginBottom:4 }}/>
          ...
          {lastDisp?.dispatch && !lastDisp.dispatch.error && (
            <div style={{ margin:'0 12px 14px', border:'1px solid var(--green-lt2)', background:'var(--green-lt)', borderRadius:9, padding:12 }}>
              ...
              {[['Patient',lastDisp.patient?.name],['Hospital',lastDisp.dispatch?.assigned_hospital?.name],['Distance',`${lastDisp.dispatch?.distance_km} km`],['ETA',`${lastDisp.dispatch?.duration_min?.toFixed(1)} min`],['Survival',`${lastDisp.dispatch?.survival_pct?.toFixed(1)}%`],['Route via',lastDisp.dispatch?.route_source?.toUpperCase()]].map(([l,v])=>(
              ...
              <div style={{ maxHeight:100, overflow:'auto', border:'1px solid var(--border)', borderRadius:7 }}>
                {traffic.map((tl,i)=>(
                  <div key={tl._id} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'4px 9px', borderBottom:i<traffic.length-1?'1px solid var(--border)':undefined }}>
                    ...
                    <span className={`badge badge-${tl.state==='green'?'green':'red'}`} style={{ fontSize:10 }}>
                      {tl.state==='green'?'● GREEN':'● RED'}
                    </span>
                  </div>
                ))}
              </div>
...
function SectionLabel({label}) {
  return (
    <div style={{ padding:'9px 12px 6px', fontSize:11.5, fontWeight:700, color:'var(--text3)', letterSpacing:'0.09em', textTransform:'uppercase', borderTop:'1px solid var(--border)', background:'var(--surface2)', fontFamily:"'Rajdhani',sans-serif" }}>
      {label}
    </div>
  )
}
