 ...
 *
 * Supports two modes:
 * 1. View mode  (default) — shows ambulances, hospitals, traffic, route
 * 2. Place mode — click on map to get lat/lon for placing entities
 *
 * Props:
 *   placeMode       : string | null  — 'ambulance' | 'hospital' | 'traffic' | 'patient' | null
 *   onMapClick      : (lat, lon) => void  — called when user clicks in place mode
 *   pendingMarker   : { lat, lon, type } | null  — shows a preview pin before confirming
 */
...
function makeTrafficIcon(state) {
  ...
  return L.divIcon({
    html: `<div style="filter:drop-shadow(0 2px 5px rgba(0,0,0,0.3))">
...
function makeEmergIcon() {
  return L.divIcon({
    html: `<div style="filter:drop-shadow(0 0 10px rgba(220,38,38,0.6))">
...
// Preview pin shown while user has selected a placement mode but not confirmed yet
const PLACE_ICONS = {
...
// Cursor overlay style when in place mode
const CURSOR_STYLES = {
  ambulance: 'crosshair',
  hospital:  'crosshair',
  traffic:   'crosshair',
...
export default function MapView({
  ...
  // ── Place mode ──────────────────────────────────
  placeMode        = null,    // 'ambulance' | 'hospital' | 'traffic' | 'patient' | null
  onMapClick       = null,    // (lat, lon) => void
  pendingMarker    = null,    // { lat, lon, type } preview pin
}) {
  ...
  useEffect(() => {
    ...
    // Change cursor
    if (m._container) {
      m._container.style.cursor = placeMode ? (CURSOR_STYLES[placeMode] || 'crosshair') : ''
    }
    return () => m.off('click', handler)
  }, [placeMode, onMapClick])

  // Preview pending marker
  useEffect(() => {
    const m = mapRef.current; if (!m) return
    ...
    if (pendingMarker?.lat && pendingMarker?.lon) {
      layers.current.pending = L.marker(
        [pendingMarker.lat, pendingMarker.lon],
        { icon: makePendingIcon(pendingMarker.type), zIndexOffset: 500 }
      )
        .bindTooltip(`Drop ${pendingMarker.type} here — confirm or click elsewhere to move`, { className: 'map-tip', permanent: false })
        .addTo(m)
    }
  }, [pendingMarker])

  // Ambulances — smooth animation
  useEffect(() => {
    const m = mapRef.current; if (!m) return
    ...
    ambulances.forEach(a => {
      ...
      if (L2[a._id]) {
        // Only rebuild icon DOM when status or label actually changes
        ...
        const icon = makeAmbIcon(a.status === 'busy', label)
        const mk = L.marker([a.lat, a.lon], { icon, zIndexOffset: 100 })
    ...
    Object.keys(L2).forEach(id => {
      if (!ambulances.find(x=>x._id===id)) {
        m.removeLayer(L2[id]); delete L2[id]
        delete prevPos.current[id]; delete prevStatus.current[id]
      }
    })
