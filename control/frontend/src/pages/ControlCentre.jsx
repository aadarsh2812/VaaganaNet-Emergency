...
export default function ControlCentre() {
  ...
  // ── Place-mode state ───────────────────────────────────────────────────────
  const [placeMode, setPlaceMode]   = useState(null)    // 'ambulance'|'hospital'|'traffic'|null
  const [pending,   setPending]     = useState(null)    // { lat, lon, type }
  ...
  // When user clicks map in place mode → show bouncing pin at that location
  const handleMapClick = (lat, lon) => {
    if (!placeMode) return
    setPending({ lat, lon, type: placeMode })
  }

  // Confirm placement — save to DB
  const confirmPlace = async () => {
    ...
    try {
      if (pending.type === 'ambulance') {
        ...
        const name = hospName || 'General Hospital'
        await addHospital(name, pending.lat, pending.lon)
        show(`🏥 ${name} placed`)
        setHospName('')
      ...
      show(e.response?.data?.detail || e.message, 'error')
    }
    setSaving(false)
  }
  ...
  return (
    <Layout title="Control Centre" subtitle="Fleet monitoring & infrastructure management">
      <div style={{ display:'flex', height:'100%', overflow:'hidden' }}>
        ...
        <div style={{ width:280, background:'white', borderRight:'1px solid var(--border)', display:'flex', flexDirection:'column', flexShrink:0, overflow:'hidden' }}>
          <div style={{ display:'flex', borderBottom:'1px solid var(--border)', background:'var(--surface2)' }}>
            {TABS.map(t => (
              <button key={t.k} onClick={() => setTab(t.k)} style={tabStyle(tab===t.k)}>{t.label}</button>
            ))}
          </div>
          ...
          <div style={{ flex:1, overflow:'auto', padding:14 }}>
            ...
            {tab === 'overview' && (
              <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
                ...
                <div>
                  ...
                  {hospitals.length === 0
                    ? <Empty msg="No hospitals on map yet" />
                    : hospitals.map(h => {
                      ...
                      return (
                        <div key={h._id} style={{ marginBottom:8 }}>
                          ...
                          <div style={{ height:5, background:'var(--surface3)', borderRadius:3 }}>
                            <div style={{ width:`${load}%`, height:'100%', background:load>80?'var(--red2)':load>50?'var(--orange2)':'var(--green2)', borderRadius:3, transition:'width 0.5s' }}/>
                          </div>
            ...
            {tab === 'infra' && (
              <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
                ...
                <PlaceCard
                  title="🚑 Add Ambulance"
                  desc="Places an idle ambulance at clicked location"
                  active={placeMode === 'ambulance'}
                  onActivate={() => { setPlaceMode('ambulance'); setPending(null) }}
                  onCancel={cancelPlace}
                >
                  <input className="input" placeholder="Label e.g. AMB-01 (optional)"
                    value={ambLabel} onChange={e => setAmbLabel(e.target.value)} />
                </PlaceCard>

                {/* Place hospital */}
                <PlaceCard
                  title="🏥 Add Hospital"
                  desc="Places a hospital at clicked location"
                  active={placeMode === 'hospital'}
                  onActivate={() => { setPlaceMode('hospital'); setPending(null) }}
                  onCancel={cancelPlace}
                >
                  <input className="input" placeholder="Hospital name"
                    value={hospName} onChange={e => setHospName(e.target.value)} />
                </PlaceCard>

                {/* Place traffic light */}
                <PlaceCard
                  title="🚦 Add Traffic Light"
                  desc="Places a traffic light at clicked location"
                  active={placeMode === 'traffic'}
                  onActivate={() => { setPlaceMode('traffic'); setPending(null) }}
                  onCancel={cancelPlace}
                >
                  <input className="input" placeholder="Intersection name (optional)"
                    value={juncName} onChange={e => setJuncName(e.target.value)} />
                </PlaceCard>
                ...
                {pending && (
                  <div style={{ background:'var(--green-lt)', border:'1px solid var(--green-lt2)', borderRadius:10, padding:12 }}>
                    ...
                    <div style={{ fontSize:12, color:'var(--text2)', marginBottom:3 }}>
                      Type: <strong>{pending.type}</strong>
                    </div>
                    <div style={{ fontSize:12, color:'var(--text2)', marginBottom:10, fontFamily:"'JetBrains Mono',monospace" }}>
                      {pending.lat.toFixed(5)}, {pending.lon.toFixed(5)}
                    </div>
                    <div style={{ display:'flex', gap:8 }}>
                      <button className="btn btn-success" onClick={confirmPlace} disabled={saving} style={{ flex:1, justifyContent:'center' }}>
                        {saving ? 'Saving…' : '✓ Confirm'}
                      </button>
                      <button className="btn btn-ghost" onClick={cancelPlace} style={{ flex:1, justifyContent:'center' }}>
                        ✕ Cancel
                      </button>
                    </div>
                  </div>
            ...
            {tab === 'log' && (
              <div>
                ...
                {arrivedLog.length === 0
                  ? <Empty msg="No completed emergencies yet" />
                  : [...arrivedLog].reverse().map((em, i) => (
                    <div key={em._id} style={{ background:'var(--green-lt)', border:'1px solid var(--green-lt2)', borderRadius:9, padding:10, marginBottom:6 }}>
                      ...
                      <div style={{ fontSize:12, color:'var(--text2)' }}>
                        {em.distance_km} km · Survival: <strong style={{ color:'var(--green)' }}>{em.survival_pct?.toFixed(0)}%</strong>
                      </div>
                    </div>
                  ))
                }
          ...
          {placeMode && (
            <div style={{ padding:'8px 16px', background:'#1d4ed8', color:'white', display:'flex', alignItems:'center', gap:10, flexShrink:0 }}>
              <span style={{ fontSize:18 }}>
                {{ ambulance:'🚑', hospital:'🏥', traffic:'🚦' }[placeMode]}
              </span>
              <span style={{ fontWeight:600, fontSize:13 }}>
                Click anywhere on the map to place the {placeMode}
              </span>
              {pending && (
                <span style={{ fontSize:12, opacity:0.8 }}>
                  · Pin dropped — confirm in the left panel
                </span>
              )}
              <button onClick={cancelPlace} style={{ marginLeft:'auto', background:'rgba(255,255,255,0.2)', border:'none', color:'white', padding:'4px 12px', borderRadius:6, cursor:'pointer', fontFamily:'inherit', fontSize:12 }}>
                Cancel
              </button>
            </div>
          )}
        ...
        <div className="overlay-backdrop" onClick={() => setSelAmb(null)}>
          <div className="animate-fade-in card" style={{ width:440, padding:0, overflow:'hidden' }} onClick={e => e.stopPropagation()}>
            <div style={{ padding:'16px 20px', background:'linear-gradient(135deg,#1e3a5f,#1d4ed8)', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
              <div>
                <div style={{ fontWeight:700, fontSize:17, color:'white', fontFamily:"'Rajdhani',sans-serif", letterSpacing:'0.02em' }}>{selAmb.label || selAmb._id.slice(0,10)}</div>
                <div style={{ fontSize:12, color:'rgba(255,255,255,0.75)', marginTop:2 }}>Vehicle Status (OBD)</div>
              </div>
              <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                <span style={{ background:'rgba(255,255,255,0.2)', color:'white', fontSize:11, fontWeight:700, padding:'3px 10px', borderRadius:20 }}>
                  {selAmb.status === 'busy' ? '● DISPATCHED' : '○ AVAILABLE'}
                </span>
            ...
            <div style={{ padding:20, display:'flex', flexDirection:'column', gap:12 }}>
              ...
              <div style={{ border:'1px solid var(--border)', borderRadius:8, padding:'10px 12px' }}>
                ...
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:6 }}>
                  <InfoRow label="Latitude"  val={selAmb.lat?.toFixed(5)} mono />
                  <InfoRow label="Longitude" val={selAmb.lon?.toFixed(5)} mono />
                </div>
              </div>
              {selAmb.assigned_patient && (
                <div style={{ background:'var(--orange-lt)', border:'1px solid var(--orange-lt2)', borderRadius:8, padding:'10px 12px' }}>
                  <div className="label" style={{ marginBottom:6 }}>Current Assignment</div>
                  <InfoRow label="Patient ID" val={selAmb.assigned_patient.slice(0,18)+'…'} mono />
                </div>
              )}
  ...
  return (
    <div style={{ border:`1.5px solid ${active ? 'var(--blue2)' : 'var(--border)'}`, background: active ? 'var(--blue-lt)' : 'white', borderRadius:10, padding:12, transition:'all 0.15s' }}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom: active ? 10 : 6 }}>
        ...
        {active
          ? <button className="btn btn-ghost btn-sm" onClick={onCancel} style={{ borderColor:'var(--red2)', color:'var(--red2)' }}>Cancel</button>
      ...
      {active && (
        <div className="animate-fade-in">
          {children}
          <div style={{ fontSize:11, color:'var(--blue2)', marginTop:8, fontWeight:500 }}>
            👆 Now click on the map to drop a pin
          </div>
        </div>
      )}
    </div>
...
function StatCard({ label, val, color, bg }) {
  return (
    <div style={{ background:bg, border:'1px solid', borderColor:color+'44', borderRadius:9, padding:'10px 12px' }}>
      <div style={{ fontFamily:"'JetBrains Mono',monospace", fontWeight:700, fontSize:22, color, lineHeight:1 }}>{val}</div>
        ...
        {[['Fuel',amb.fuel,100,'%','var(--green2)'],['Batt',amb.battery,100,'%','var(--blue2)'],['Temp',amb.engine_temp,120,'°','var(--orange2)']].map(([l,v,m,u,c])=>(
          <div key={l} style={{ flex:1 }}>
            <div style={{ display:'flex', justifyContent:'space-between', marginBottom:2 }}>
              ...
              <span style={{ fontSize:10, fontWeight:600, color:c, fontFamily:"'JetBrains Mono',monospace" }}>{v?.toFixed(0)}{u}</span>
            </div>
            <div style={{ height:3, background:'var(--surface3)', borderRadius:2 }}>
              <div style={{ width:`${Math.min(100,(v/m)*100)}%`, height:'100%', background:c, borderRadius:2 }}/>
            </div>
          </div>
        ))}
      </div>
      <div style={{ fontSize:10, color:'var(--text3)', marginTop:6 }}>Click to view OBD details</div>
    </div>
  )
}
...
function OBDGauge({ label, val, max, unit, lo, hi, color }) {
  ...
  return (
    <div style={{ border:`1px solid ${isWarn?'var(--red-lt2)':'var(--border)'}`, background:isWarn?'var(--red-lt)':'white', borderRadius:9, padding:'10px 12px', textAlign:'center' }}>
      ...
      <div style={{ fontFamily:"'JetBrains Mono',monospace", fontWeight:700, fontSize:20, color:c, lineHeight:1, marginBottom:2 }}>
        {val?.toFixed(0)}<span style={{ fontSize:11, fontWeight:400 }}>{unit}</span>
      </div>
      {isWarn && <div style={{ fontSize:10, color:'var(--red)', fontWeight:600, marginBottom:4 }}>⚠ Warning</div>}
      <div style={{ height:5, background:'var(--surface3)', borderRadius:3, marginTop:6 }}>
...
function InfoBox({ label, val, color }) {
  return (
    <div style={{ background:'var(--surface2)', border:'1px solid var(--border)', borderRadius:7, padding:'8px 10px', textAlign:'center' }}>
      <div style={{ fontFamily:"'JetBrains Mono',monospace", fontWeight:700, fontSize:18, color }}>{val}</div>
...
function InfoRow({ label, val, mono }) {
  return (
    <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
      ...
      <span style={{ fontSize:12, fontWeight:500, color:'var(--text)', fontFamily:mono?"'JetBrains Mono',monospace":undefined }}>{val}</span>
    </div>
  )
}
...
function QuickStat({ label, val, color='var(--text2)' }) {
  return (
    <div style={{ textAlign:'center' }}>
      <div style={{ fontFamily:"'JetBrains Mono',monospace", fontWeight:700, fontSize:18, color, lineHeight:1 }}>{val}</div>
      <div style={{ fontSize:10, color:'var(--text3)', marginTop:2 }}>{label}</div>
    </div>
  )
}
